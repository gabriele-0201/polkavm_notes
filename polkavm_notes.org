...
9. iterate over `pc_to_label` field
   ...
   for (pc, label) in self.pc_to_label.drain() {
    ...
    #+end_src
10. Update `export_trampolines`

    translation of the exports address to the native_address (not address of the exports but to the label that should point to the final real export) (TODO: Are you sure? Is this even english?)
    #+begin_src rust
