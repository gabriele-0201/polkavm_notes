...
| index             | before each function there is a varint encoding the index of the following fn * n_exports |
...
2. set the origin of the code in the `Assembler`
   ...
   Why the origin of the code in the Virtual Machine is set up at 4GiB? (TODO)
3. fetch the next_instruction from the `instrucitons` array
4. get the current length of the code -> `initial_length = self.asm.len()`
5. fetch the current_instruction from the `instrucitons` array
6. if trace_execution is enabled and the current_instruction is NOT an Opcode::jump_target
   1. `self.trace_execution(nth_instruction);` -> (TODO)
7. `instruction.visit(self).map_err(Error::from_static_str)?;`
   1. self=Compiler -> `Compiler` implements `InstructionVisitor`, this means that for each instruction is possible to implement custom logic accepting the `Compiler` as arguments

      How? because thmethod `visit` for each instruction, in the macro for each variant of the Opcode (inside the RawInstruction) there is a call to `visitor.name_of_instruction` where in this case the visitor is the `Compiler`

8. TODO !??!?!?? or is it finished?
